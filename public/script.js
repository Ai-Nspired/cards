...
User speaks naturally. Build the doc.`;
  ...
  function exportData() { const d = { cards: app.cards, theme: app.theme, settings: app.settings, sessionId: app.sid, userId: app.uid }; const b = new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'cards_' + Date.now() + '.json'; a.click(); toast('Exported'); }

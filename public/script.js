...
User speaks naturally. Build the doc.`;
  ...
  const to = (m) => { const t = el('toast'); t.textContent = m; t.classList.add('show'); setTimeout(() => t.classList.remove('show'), 1800); };
  ...
  const app = {
    ...
    proxyUrl() { const v = el('proxyUrl')?.value.trim(); if (v) { this.settings.proxyUrl = v; this.save(); return v; } return this.settings.proxyUrl || 'https://ai.nspired.cc/chat'; },
    ...
    async pipeChat(msgs, el) {
      ...
      try {
        const res = await fetch(`${this.proxyUrl()}/chat`, {
          ...
          body: JSON.stringify({ messages: msgs, model: 'inclusionai/ling-2.6-flash', stream: true, max_tokens: 2000 })
        });
    ...
    loadStyle(st) {
      ...
      el('stPadV').textContent = parseInt(st.padding || '14');
      el('stRadV').textContent = parseInt(st.borderRadius || '12');
    },
    saveStyle() {
      if (!this.stId) return;
      const s = {
  ...
  function toast(m) { const t = el('toast'); if (!t) { const nt = document.createElement('div'); nt.id = 'toast'; nt.textContent = m; nt.classList.add('show'); document.body.appendChild(nt); setTimeout(() => nt.classList.remove('show'), 1800); return; } t.textContent = m; t.classList.add('show'); clearTimeout(el('_tt')); el('_tt') = setTimeout(() => t.classList.remove('show'), 1800); }
  function exportData() { const d = { cards: app.cards, theme: app.theme, settings: app.settings, sessionId: app.sid, userId: app.uid }; const b = new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'ncards_' + Date.now() + '.json'; a.click(); toast('Exported'); }
  function importData(e) { const f = e.target.files[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { try { const d = JSON.parse(ev.target.result); app.pushHist('Import'); app.cards = d.cards || []; app.theme = { ...app.theme, ...d.theme }; app.settings = { ...app.settings, ...d.settings }; app.sid = d.sessionId || null; app.uid = d.userId || app.uid; app.save(); app.render(); app.applyTheme(); toast('Imported'); app.closeModal('Proxy'); } catch { alert('Invalid JSON'); } }; r.readAsText(f); e.target.value = ''; }

  // Modal operations using injected container and a single shared container
  function openModal(id) {
    ...
    if (id === 'modalProxy') {
      const wrap = document.createElement('div'); wrap.className = 'overlay show'; wrap.id = 'modalProxy';
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') {
      el('#modalProxy')?.remove(); el('#modalStyle')?.remove(); el('#modalPrompt')?.remove(); el('#modalFullscreen')?.remove();
      app.closeCtx();
    }
  });
  ...
  // modal control bindings (stable)
  el('#closeProxy')?.addEventListener('click', () => closeModal('modalProxy'));
  el('#closeProxyM')?.addEventListener('click', () => closeModal('modalProxy'));
  el('#saveProxy')?.addEventListener('click', () => { app.settings.proxyUrl = app.proxyUrl(); app.save(); });
  el('#resetProxy')?.addEventListener('click', () => { el('#proxyUrl').value = 'https://ai.nspired.cc/chat'; app.settings.proxyUrl = ''; app.save(); });
  el('#viewList')?.addEventListener('click', () => app.setView('list'));
  el('#viewGrid')?.addEventListener('click', () => app.setView('grid'));
  el('#viewFull')?.addEventListener('click', () => app.setView('full'));
  el('#saveStyle')?.addEventListener('click', () => app.saveStyle());
  el('#fsClose')?.addEventListener('click', () => closeModal('modalFullscreen'));
  el('#fsFlipBtn')?.addEventListener('click', () => app.toggleFsFlip());
  el('#fsReadBtn')?.addEventListener('click', () => app.toggleNative());

  el('#cancelSel')?.addEventListener('click', () => app.clearSel());
  el('#deleteSel')?.addEventListener('click', () => app.bulkDel());
  el('#mergeSel')?.addEventListener('click', () => app.prompt('merge'));
  el('#splitSel')?.addEventListener('click', () => app.prompt('split'));
  el('#continueSel')?.addEventListener('click', () => app.prompt('continue'));

  el('promptGo')?.addEventListener('click', () => app.execPrompt());
  el('closePrompt')?.addEventListener('click', () => closeModal('modalPrompt'));
  el('promptArea')?.addEventListener('keydown', e => { if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey) { e.preventDefault(); app.execPrompt(); } });

  el('importFile')?.addEventListener('change', e => importData(e));

  document.querySelectorAll('.overlay').forEach(o => {
    o.addEventListener('click', e => { if (e.target === e.currentTarget) closeModal(o.id); });
  });
  document.querySelectorAll('.tgl').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.id;
      if (id === 'lockStyle') app.saveStyle();
      else if (id === 'viewList') app.setView('list');
      else if (id === 'viewGrid') app.setView('grid');
      else if (id === 'viewFull') app.setView('full');
  ...
  document.querySelectorAll('.icon-btn[title]').forEach(btn => {
    let ttip;
    btn.addEventListener('mouseenter', () => { ttip = document.createElement('span'); ttip.textContent = btn.title; ttip.style.cssText = 'position:fixed;background:#333;color:#fff;padding:4px 8px;border-radius:4px;font-size:12px;z-index:9999;white-space:nowrap;'; document.body.appendChild(ttip); });
    btn.addEventListener('mousemove', (e) => { if (ttip) { ttip.style.left = (e.clientX + 12) + 'px'; ttip.style.top = (e.clientY + 12) + 'px'; } });
    btn.addEventListener('mouseleave', () => { ttip && ttip.remove(); ttip = null; });
  });
})();